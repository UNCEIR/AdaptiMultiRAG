  ...
  <div class="h-screen flex bg-amber-50/30">
    ...
    <div class="w-80 bg-white border-r border-gray-100 flex flex-col shadow-sm">
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <!-- 加载状态 -->
        <div v-if="loading && documentLibraries.length === 0" class="flex items-center justify-center py-8">
          <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
          <span class="ml-2 text-gray-600 font-light">加载中...</span>
        </div>

        <!-- 错误状态 -->
        <div v-else-if="error && documentLibraries.length === 0" class="text-center py-8">
        ...
        <!-- 空状态 -->
        <div v-else-if="documentLibraries.length === 0" class="text-center py-8 text-gray-500">
        ...
        <div v-else class="space-y-2">
          <div
            ...
            :class="{
              'bg-amber-50 border-amber-200': selectedLibrary?.id === library.id,
              'hover:bg-gray-50 border-gray-100': selectedLibrary?.id !== library.id
            }"
          >
            <div class="flex items-start justify-between">
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center mt-2 text-xs text-gray-400 font-light">
    ...
    <div class="flex-1 flex flex-col">
      <!-- 文档库详情头部 -->
      <div class="bg-white/80 backdrop-blur-sm border-b border-gray-100 p-4 shadow-sm" v-if="selectedLibrary">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-lg font-normal text-gray-900">{{ selectedLibrary.title }}</h2>
            <p class="text-sm text-gray-600 mt-1 font-light">{{ selectedLibrary.description || '暂无描述' }}</p>
          </div>
      ...
      <div class="flex-1 overflow-y-auto p-4" v-if="selectedLibrary">
        <!-- 文档加载状态 -->
        <div v-if="documentLoading" class="flex items-center justify-center py-8">
          <div class="animate-spin rounded-full h-6 w-6 border-b-2 border-gray-900"></div>
          <span class="ml-2 text-gray-600 font-light">加载文档中...</span>
        </div>

        <!-- 文档列表 -->
        <div v-else-if="selectedLibrary.documents && selectedLibrary.documents.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            ...
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center mr-3"
                  ...
                  <svg v-if="document.type === 'link'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="text-sm font-normal text-gray-900 truncate">{{ document.name }}</h4>
                  <p class="text-xs text-gray-500 font-light">{{ document.type === 'link' ? '网站链接' : '文件' }}</p>
                </div>
              </div>
              <button
      ...
      <div v-else class="flex-1 flex items-center justify-center text-gray-500">
        <div class="text-center">
          ...
          <p class="text-sm font-light">从左侧列表中选择文档库来查看和管理文档</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCreateDialog || showEditDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-gray-100 p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-normal text-gray-900 mb-4">
          {{ showEditDialog ? '编辑文档库' : '创建文档库' }}
        </h3>
        <form @submit.prevent="showEditDialog ? updateLibrary() : createLibrary()">
          <div class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2">标题</label>
            <input
          ...
          <div class="mb-6">
            <label class="block text-sm font-normal text-gray-700 mb-2">描述</label>
            <textarea
    ...
    <!-- 添加文档对话框 -->
    <div v-if="showAddDocumentDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-gray-100 p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-normal text-gray-900 mb-4">添加文档</h3>
        <form @submit.prevent="addDocument">
          <div class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2">文档类型</label>
            <select
          ...
          <div v-if="documentForm.type === 'link'" class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2">链接地址</label>
            <input
          ...
          <div v-if="documentForm.type === 'link'" class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2">URL前缀限制</label>
            <input
            ...
            />
            <p class="text-xs text-gray-500 mt-1 font-light">只爬取以此前缀开头的URL，留空则自动使用输入URL的根路径</p>
          </div>

          <div v-if="documentForm.type === 'file'" class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2">选择文件</label>
            <input
              ...
              class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent disabled:opacity-50 bg-white"
            />
            <p class="text-xs text-gray-500 mt-1 font-light">支持 PDF、DOC、DOCX、MD、TXT 格式</p>
          </div>

          <div class="mb-6">
            <label class="block text-sm font-normal text-gray-700 mb-2">文档名称</label>
            <input
...
const updateLibrary = async () => {
  try {
    loading.value = true
    const response = await knowledgeAPI.updateLibrary(selectedLibrary.value.id, {
      title: libraryForm.title,
      description: libraryForm.description
    })
...
const addDocument = async () => {
  try {
    ...
    if (documentForm.type === 'file' && documentForm.file) {
      try {
        // 1. 获取OSS上传签名URL
        const uploadResponse = await knowledgeAPI.getUploadUrl(documentForm.file.name)

<template>
  <div class="h-screen flex bg-amber-50/30">
      ...
      <div class="p-4 border-b border-gray-100">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center space-x-2">
            ...
            <h1 class="text-lg font-medium text-gray-900">AI Assistant</h1>
          </div>
          <button
            @click="logout"
      ...
      <div class="flex-1 overflow-y-auto p-3">
        <div class="space-y-1">
          <div
            ...
            :class="{
              'bg-amber-50 border border-amber-200': currentConversation?.id === conversation.id,
              'hover:bg-gray-50 border border-transparent': currentConversation?.id !== conversation.id
            }"
          >
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <!-- 聊天头部 -->
      <div class="bg-white/80 backdrop-blur-sm border-b border-gray-100 p-4 shadow-sm">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4" ref="messagesContainer">
        <!-- 加载状态 -->
        <div v-if="loading && currentConversation" class="flex items-center justify-center h-full">
          <div class="text-center">
            <div class="relative inline-block">
              <!-- 背景圆环 -->
              <div class="w-12 h-12 rounded-full border-4 border-gray-200"></div>
              <!-- 旋转的蓝色圆环 -->
              <div class="absolute inset-0 w-12 h-12 rounded-full border-4 border-transparent border-t-blue-600 animate-spin"></div>
            </div>
        ...
        >
          ...
          <div v-if="message.role === 'node_update'" class="w-full max-w-3xl">
            <details class="node-update-details">
              ...
              <div class="node-update-content">
                <div class="node-update-content-text">{{ message.content }}</div>
              </div>
            </details>
          </div>
          ...
          <template v-else>
            ...
            <template v-if="message.role === 'assistant'">
              ...
              <div class="flex-1 max-w-3xl">
                ...
                <div class="flex items-center gap-2 mt-3">
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="编辑">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </button>
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="复制">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                    </svg>
                  </button>
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="朗读">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                    </svg>
                  </button>
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="点赞">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
                    </svg>
                  </button>
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="点踩">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.736 3h4.018a2 2 0 01.485.06l3.76.94m-7 10v5a2 2 0 002 2h.096c.5 0 .905-.405.905-.904 0-.715.211-1.413.608-2.008L17 13V4m-7 10h2m5-10h2a2 2 0 012 2v6a2 2 0 01-2 2h-2.5" />
                    </svg>
                  </button>
                  <button class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-100 transition-colors" title="重新生成">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- 用户消息 -->
            <template v-else>
              <div class="px-4 py-2 rounded-lg bg-gray-900 text-white max-w-xs lg:max-w-md">
    ...
    >
      ...
      >
        <!-- 可视化拖动指示器 -->
        <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-12 bg-gray-300 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
      </div>
...
flowchart TD
    ...
    C --> n6
        ...
        <!-- 当前执行节点提示 -->
        <div v-if="currentExecutingNode" class="mt-4 p-3 bg-white/80 border border-gray-200 rounded-lg shadow-sm">
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 bg-gray-900 rounded-full animate-pulse"></div>
            <span class="text-sm font-normal text-gray-900">正在执行: {{ currentExecutingNode }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div class="space-y-4">
        <div class="flex items-start space-x-3 p-4 bg-amber-50 rounded-lg border border-amber-200">
          <svg class="w-5 h-5 text-gray-700 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div class="text-sm text-gray-800">
            <p class="font-normal mb-1">提示</p>
            <p class="font-light">选择一个知识库以获得更准确的回答，或选择"不使用知识库"直接与AI对话。</p>
          </div>
        </div>

        <div>
          <label class="block text-sm font-normal text-gray-700 mb-3">请选择知识库</label>
          <div v-if="librariesLoading" class="text-center py-8">
            <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
            <p class="text-sm text-gray-500 mt-3 font-light">正在加载知识库...</p>
          </div>
          <div v-else-if="knowledgeLibraries.length === 0" class="text-center py-8">
            <EmptyState
              title="暂无知识库"
          ...
          <div v-else class="space-y-2 max-h-64 overflow-y-auto">
            ...
            >
              ...
              />
              <div class="ml-3">
                <div class="text-sm font-normal text-gray-900">不使用知识库</div>
                <div class="text-xs text-gray-500 font-light">仅使用基础AI模型回答</div>
              </div>
            </label>
            <label
              ...
              class="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-amber-50/50 transition-all duration-200"
              :class="{
                'border-gray-900 bg-amber-50': selectedLibraryForDialog === library.id,
            ...
            >
              ...
              <div class="ml-3 flex-1">
                <div class="text-sm font-normal text-gray-900">{{ library.title }}</div>
                <div class="text-xs text-gray-500 font-light">{{ library.description || '暂无描述' }}</div>
              </div>
        ...
        <div class="flex items-center space-x-2 text-xs text-gray-500">
          ...
          />
          <label for="dontShowAgain" class="cursor-pointer font-light">不再显示此提示</label>
        </div>
      </div>
    ...
    >
      ...
      >
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 class="text-lg font-normal text-gray-900">RAG设置</h3>
          <button
            @click="closeSettingsModal"
        ...
        <div class="p-6 space-y-6">
          <!-- RAG模式选择 -->
          <div>
            <label class="block text-sm font-normal text-gray-700 mb-3">RAG模式</label>
            <div class="space-y-2">
              <label
                ...
                :class="{ 'border-gray-900 bg-amber-50': ragMode === option.value }"
              >
                <input
                  type="radio"
                  :value="option.value"
                ...
                />
                <div class="ml-3">
                  <div class="text-sm font-normal text-gray-900">{{ option.label }}</div>
                  <div class="text-xs text-gray-500 font-light">{{ getRagModeDescription(option.value) }}</div>
                </div>
              </label>
            </div>
          </div>
          ...
          <!-- 知识库选择 -->
          <div>
            <label class="block text-sm font-normal text-gray-700 mb-3">选择知识库</label>
            <div v-if="librariesLoading" class="text-center py-4">
              <div class="inline-block animate-spin rounded-full h-6 w-6 border-b-2 border-gray-900"></div>
              <p class="text-sm text-gray-500 mt-2 font-light">正在加载知识库...</p>
            </div>
            <div v-else-if="knowledgeLibraries.length === 0" class="text-center py-4">
              <p class="text-sm text-gray-500 font-light">暂无知识库</p>
            </div>
            <div v-else class="space-y-2 max-h-48 overflow-y-auto">
              <label
                class="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-amber-50/50 transition-colors"
                :class="{ 'border-gray-900 bg-amber-50': selectedLibrary === '' }"
              >
                <input
                  type="radio"
                  value=""
              ...
              <label
                ...
                :key="library.id"
                class="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-amber-50/50 transition-colors"
                :class="{ 'border-gray-900 bg-amber-50': selectedLibrary === library.id }"
              >
                <input
                  type="radio"
                  :value="library.id"
          ...
          <div class="border-t border-gray-100 pt-6">
            ...
            <div class="mb-4">
              ...
              />
              <div class="flex justify-between text-xs text-gray-500 mt-1 font-light">
                <span>1</span>
                <span>10</span>
              </div>
            </div>
            ...
            <!-- 系统提示词 -->
            <div>
              <label class="block text-sm font-normal text-gray-700 mb-2">系统提示词</label>
              <textarea
...
const systemPrompt = ref('你是一个专业的RAG助手，能够基于检索到的信息提供准确的回答。') // 系统提示词
...
const highlightNode = (nodeName, retryCount = 0) => {
  ...
  console.log('🎯 准备高亮节点:', nodeName, '-> ID:', nodeId, retryCount > 0 ? `(重试${retryCount})` : '')
  ...
  if (!currentNodeGroup) {
    ...
    const allNodes = svg.querySelectorAll('g.node, g.node.default, g[class*="node"]')
  ...
  if (currentNodeGroup) {
    ...
    if (retryCount < 3) {
      ...
      const allNodes = Array.from(svg.querySelectorAll('g.node, g[class*="node"]'))
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (selectedLibrary.value) {
      const collectionId = getSelectedLibraryCollectionId(selectedLibrary.value)
      if (collectionId) {
...
watch(messages, (newMessages, oldMessages) => {
  ...
  // 检查最后一条AI消息的内容变化
  const lastAiMessage = newMessages.filter(m => m.role === 'assistant').pop()
  if (lastAiMessage) {
    console.log('  最后一条AI消息内容长度:', lastAiMessage.content.length)
    console.log('  最后一条AI消息内容预览:', lastAiMessage.content.substring(0, 50) + '...')
  }
...
watch(() => messages.value.map(m => m.content), (newContents, oldContents) => {
...
const resetSettings = () => {
  ...
  systemPrompt.value = '你是一个专业的RAG助手，能够基于检索到的信息提供准确的回答。'
}
